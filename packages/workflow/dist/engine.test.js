...
describe("Engine.graph", () => {
    ...
    it("validates that there are no cycles or self-references", () => {
        ...
        expect(() => engine.graph(wf)).toThrow("Workflow instance must be a DAG;  the given workflow has at least one cycle.");
    });
...
describe("ExecutionState.interpolate", () => {
    it("correctly references current state and events", () => {
        let state = new engine_1.ExecutionState({
            event: {
                data: {
                    userId: 1.1,
                    likes: ["a"],
        ...
        expect(state.interpolate(`{"==": [{"var": "!ref($.state.action_a)"}, "a"}`)).toEqual(`{"==": [{"var": "test"}, "a"}`);
        ...
        expect(state.interpolate([123])).toEqual([123]);
        // Object walking.
        expect(state.interpolate({ "==": ["!ref($.state.action_b)", "a"] })).toEqual({ "==": [{ ok: true }, "a"] });
